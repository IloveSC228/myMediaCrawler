...
class KuaishouCsvStoreImplement(AbstractStore):
    ...
    def make_save_file_name(self, store_type: str) -> str:
        ...
        """
        return f"{self.csv_store_path}/ks_{self.file_count}_{crawler_type_var.get()}_{store_type}_{utils.get_current_date()}.csv"
    ...
    async def store_extract_info(self, extract_info_item: Dict):
        return await super().store_extract_info(extract_info_item)
    
    async def store_pic(self, pic_item):
        return await super().store_pic(pic_item)
    
    async def store_video(self, video_item):
        return await super().store_video(video_item)

class KuaishouDbStoreImplement(AbstractStore):
    async def store_hotlist(self, hotlist_item: Dict):
        """
        Kuaishou hotlist mysqlDB storage implementation
        Args:
            content_item: hotlist item dict

        Returns:

        """

        from .kuaishou_store_sql import (add_new_hotlist,
    ...
    async def store_content(self, content_item: Dict):
        ...
        video_detail: Dict = await query_content_by_content_id(content_id=content_id)
        if not video_detail:
    ...
    async def store_extract_info(self, extract_info_item: Dict):
        """
        Kuaishou extract_info DB storage implementation
        Args:
        ...
        """
        from .kuaishou_store_sql import (add_new_extract_info,
                                         query_extract_info,
                                         update_extract_info)
    ...
    async def store_video(self, video_item):
        ...
        return await async_db_conn.insert("video", video_item)
