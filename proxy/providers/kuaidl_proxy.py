...
class KuaiDaiLiProxy(ProxyProvider):
    def __init__(self, kdl_user_name: str, kdl_user_pwd: str, kdl_secret_id: str, kdl_signature: str):
        ...
        self.logger = utils.get_logger(config.PLATFORM)
    ...
    async def get_proxies(self, num: int) -> List[IpInfoModel]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                self.logger.error(f"[KuaiDaiLiProxy.get_proxies] statuc code not 200 and response.txt:{response.text}")
            ...
            if ip_response.get("code") != 0:
                self.logger.error(f"[KuaiDaiLiProxy.get_proxies]  code not 0 and msg:{ip_response.get('msg')}")
