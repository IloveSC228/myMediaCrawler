...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def check_login_state(self, no_logged_in_session: str) -> bool:
        ...
        if "请通过验证" in await self.context_page.content():
            self.logger.info("登录过程中出现验证码，请手动验证")
    ...
    async def login_by_mobile(self):
        ...
        while max_get_sms_code_time > 0:
            self.logger.info(f"[XiaoHongShuLogin.login_by_mobile] get sms code from redis remaining time {max_get_sms_code_time}s ...")
    ...
    async def login_by_qrcode(self):
        ...
        if not base64_qrcode_img:
            self.logger.error("小红书爬虫未能找到登录二维码")
    ...
    async def login_by_cookies(self):
        ...
        self.logger.info("小红书爬虫cookie登录")
        ...
        self.logger.info("小红书爬虫cookie登录成功")