...
const {JSDOM} = jsdom;
const dom = new JSDOM(`<!DOCTYPE html><html lang="cn"><head></head><body></body></html>`, {url: 'https://www.zhihu.com/'});
window = dom.window;
...
function ed(u){
    var loash = fff(1514)
        var ta = "101_3_3.0"
        , tu = "AWCSZWrk1BiPTpdbc4XhCLvp3Jwbt1Nt0zo=|1719383817"
        , tc = ''
        , tf = u
        , td = ''
        , tp = [ta, tf, tu, V(td) && td, tc].filter(Boolean).join("+")
      ,encrypted = md5Hash(tp);
      console.log(encrypted)

    return loash.ZP(encrypted)
}

console.log(ed('/api/v4/search/customize'))
