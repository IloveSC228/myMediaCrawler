...
class ZhiHuLogin(AbstractLogin):
    ...
    async def check_login_state(self, no_logged_in_session: str) -> bool:
        ...
        current_web_session = cookie_dict.get("SESSIONID")
    ...
    async def begin(self):
        ...
        if config.LOGIN_TYPE == "qrcode":
            ...
            raise ValueError("[ZhiHuLogin.begin]Invalid Login Type Currently only supported qrcode or phone or cookies ...")

    async def login_by_mobile(self):
        """Login ZhiHu by mobile"""
        self.logger.info("知乎平台手机登录")
        pass
        ...
        #     self.logger.error(f"未能找到手机登录按键: {e}")
        #     # utils.logger.info("[ZhiHuLogin.login_by_mobile] have not found mobile button icon and keep going ...")
        ...
        #     self.logger.info(f"[ZhiHuLogin.login_by_mobile] get sms code from redis remaining time {max_get_sms_code_time}s ...")
        #     # utils.logger.info(f"[ZhiHuLogin.login_by_mobile] get sms code from redis remaining time {max_get_sms_code_time}s ...")
        #     await asyncio.sleep(1)
        #     sms_code_key = f"zh_{self.login_phone}"
        ...
        # utils.logger.info(f"[ZhiHuLogin.login_by_mobile] Login successful then wait for {wait_redirect_seconds} seconds redirect ...")
        # await asyncio.sleep(wait_redirect_seconds)

    async def login_by_qrcode(self):
        """login ZhiHu website and keep webdriver login state"""
        self.logger.info("知乎二维码登录")
        ...
        if not base64_qrcode_img:
            self.logger.error("知乎爬虫未能找到登录二维码")
            # utils.logger.info("[ZhiHuLogin.login_by_qrcode] login failed , have not found qrcode please check ....")
            # if this website does not automatically popup login dialog box, we will manual click login button
            await asyncio.sleep(0.5)
            login_button_ele = self.context_page.locator("xpath//*[@id='root']/div/main/div/div/div/div/div[2]/div/div[1]/div/div[1]/form/button")
            await login_button_ele.click()
            base64_qrcode_img = await utils.find_login_qrcode(
                self.context_page,
                selector=qrcode_img_selector
        ...
        self.logger.info("等待2min扫描二维码登录")
        # utils.logger.info(f"[ZhiHuLogin.login_by_qrcode] waiting for scan code login, remaining time is 120s")
        try:
            await self.check_login_state(no_logged_in_session)
        except RetryError:
            self.logger.error("知乎爬虫登录失败")
            # utils.logger.info("[ZhiHuLogin.login_by_qrcode] Login ZhiHu failed by qrcode login method ...")
            sys.exit()

        wait_redirect_seconds = 5
        self.logger.info("知乎爬虫登录成功")
        # utils.logger.info(f"[ZhiHuLogin.login_by_qrcode] Login successful then wait for {wait_redirect_seconds} seconds redirect ...")
        await asyncio.sleep(wait_redirect_seconds)

    async def login_by_cookies(self):
        """login ZhiHu website by cookies"""
        self.logger.info("知乎爬虫cookie登录")
        ...
        self.logger.info("知乎爬虫cookie登录成功")